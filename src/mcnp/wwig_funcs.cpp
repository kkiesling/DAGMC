...
void wwiginit_(char* cfile, int* clen,  // geom
  ...
  raystat_dump = new std::ofstream("wwig_raystat_dump.csv");
#endif

  *wwig_version = DAG->version();
...
void wwigwritemcnp_(char* dagfile, char* lfile, int* llen, const char* mcnp_version_major) { // file with cell/surface cards
...
void wwig_bank_usetop_() {
  ...
  if (history_bank.size()) {
    ...
    std::cerr << "wwig_bank_usetop_() called without bank history!" << std::endl;
  }
}
...
void wwig_set_settings_(int* fort_use_dist_limit, int* use_cad, double* overlap_thickness, int* srccell_mode) {
...
void wwig_init_settings_(int* fort_use_dist_limit, int* use_cad,
